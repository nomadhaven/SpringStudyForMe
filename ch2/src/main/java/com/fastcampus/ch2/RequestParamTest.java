...
//		http://localhost/ch2/requestParam3         ---->> year=null   400 Bad Request. required=true라서  필수 값을 안 넣어서 client error로 인식
...
//		http://localhost/ch2/requestParam7?year   ---->> 400 Bad Request, nested exception is java.lang.NumberFormatException: For input string: "" 
	...
	//필수 입력이 아니라서 값을 입력하지 않아도 클라이언트가 잘못한게 아님 따라서 디폴트 밸류없이 실행하면 500 오류가 난다.
	//defualtValuer를 넣고 실행하면 웹 화면상에선 year=null로 나오지만 콘솔 상에선 [Sat Apr 09 16:17:18 KST 2022]year=[2021] 로 출력
	//사용자는 아무런 값도 입력하지 않아서 화면에선 null이 됐지만 디폴트 밸류는 2021로 설정했으므로 콘솔에서 2021이란 값을 인식하는 것.	
	...
	//year이란 문자 값을 입력했으므로 입력값인 int year과 다른 변수 타입이다.  문자열을 정수형으로 인식하지 못해서 에러가 난다.
